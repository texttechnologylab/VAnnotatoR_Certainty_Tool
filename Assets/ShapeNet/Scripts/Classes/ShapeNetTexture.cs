...
public class ShapeNetTexture : ShapeNetObject
{
    ...
    {
        if (!SceneController.GetInterface<ShapeNetInterface>().CachedTexturePathMap.ContainsKey((string)ID) && !_requested)
        {
            _requested = true;
        ...
        while (!SceneController.GetInterface<ShapeNetInterface>().CachedTexturePathMap.ContainsKey((string)ID))
            yield return null;

        Material = TextureLoader.LoadMaterialFile(SceneController.GetInterface<ShapeNetInterface>().CachedTexturePathMap[(string)ID] + "\\", Files);
